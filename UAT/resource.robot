...
#	\    ${elem}=    Get Text   dom=document.getElementById('sortdate_0')
	...
	\	 ${sec_status}=    Get Element Attribute    id=status_1@value
	\	 Continue For Loop If    '${status}'=='${EMPTY}'
	\    Run Keyword If    '${status}'=='status=public_enable_download'    Should Contain    ${fir_status}    Public & Enable Download
	\    Run Keyword If    '${status}'=='status=public_enable_download'    Should Contain    ${sec_status}    Public & Enable Download
	\    Run Keyword If    '${status}'=='status=public_disable_download'    Should Contain    ${fir_status}    Public & Disable Download
	\    Run Keyword If    '${status}'=='status=public_disable_download'    Should Contain    ${sec_status}    Public & Disable Download
	\    Run Keyword If    '${status}'=='status=private'    Should Contain    ${fir_status}    Private
	\    Run Keyword If    '${status}'=='status=private'    Should Contain    ${sec_status}    Private
